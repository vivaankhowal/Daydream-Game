...
func _physics_process(delta):
	...
	# Jump
	if Input.is_action_just_pressed("p2_up") and is_on_floor() and not rolling:
		velocity.y = JUMP_VELOCITY
		anim.play("jump")

	# Roll
	if Input.is_action_just_pressed("p2_roll") and not rolling:
		start_roll()

	# Attack
	if Input.is_action_just_pressed("p2_attack") and not rolling:
		play_hit()
	...
	if not rolling:
		...
		if direction != 0:
			...
			if is_on_floor() and not Input.is_action_pressed("p2_attack") and not hurting:
				anim.play("run")
		else:
